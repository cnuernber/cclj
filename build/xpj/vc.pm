...
my $visual_studio_version_info = { 
	build_tools_version=>"4.0",
...
END
		...
		foreach my $target (@$targets) {
			...
			print $targetProj 
			"<Project DefaultTargets=\"Build\" ToolsVersion=\"$visual_studio_version_info->{build_tools_version}\" xmlns=\"http://schemas.microsoft.com/developer/msbuild/2003\">\n";
...
END
			...
			foreach my $conf (@$targetConfigs) {
				...
				print $targetProj "    <PlatformToolset>$visual_studio_version_info->{platform_toolset}</PlatformToolset>\n";
...
return { process=>\&process
		, extended_compilation_properties=>$compilation_properties
		, visual_studio_version_info=>$visual_studio_version_info };